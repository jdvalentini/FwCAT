...
function parseNAT(line,hierarchy,object){               // Receives the configuration line for a NAT and the object affected if applicable
    ...
    object = object || false
    var nat = {}
    nat['srcInterface'] = line.split('(')[1].split(',')[0].trim()
    nat['dstInterface'] = line.split(',')[1].split(')')[0].trim()

    nat.options = []
    var args = line.split(')')[1].trim().split(' ')
    ...
    if (!object) {
        ...
        while (args.length > 0){
            if (/source/.test(args[0])){
                nat.source = {type:args[1], real:args[2], syntax:'implicit'}
                if (/interface/.test(args[3])) {
                    nat.source.syntax = 'interface'
                    nat.source.translated = 'interface'
                    if (args[4] == 'ipv6') {
                        nat.source.IPv = 6
                        args.shift()
                    }
                    else nat.source.IPv = 4
                    args.shift()
                }
                else if (!(/destination|service|net-to-net|dns|unidirectional|no-proxy-arp|inactive|description/).test(args[3])) {
                    nat.source.translated = args[3]
                    nat.source.syntax = 'object'
                    args.shift()
                }
                args = args.slice(3)
            }
            else if (/destination/.test(args[0])){
                nat.destination = {type:args[1]}
                if (/interface/.test(args[2])) {
                    nat.destination.syntax = 'interface'
                    nat.destination.translated = 'interface'
                    if (args[3] == 'ipv6') {
                        nat.destination.IPv = 6
                        args.shift()
                    }
                    else nat.destination.IPv = 4
                    args.shift()
                }
                else {
                    nat.destination.translated = args[2]
                    nat.destination.syntax = 'object'
                    args.shift()
                }
                nat.destination.real = args[2]
                args = args.slice(3)
            }
            else if (/service/.test(args[0])){
                nat[args[0]] = {real:args[1], translated:args[2]}
                args = args.slice(3)
            }
            else if (/description/.test(args[0])){
                nat.comment = args.slice(1).join(' ')
                args = []
            }
            else if (/net-to-net|no-proxy-arp|unidirectionl|inactive|dns/.test(args[0])) {
                nat.options.push(args[0])
                args.shift()
            }
            else {args.shift()}
        }
    ...
    else { // nat [( real_ifc, mapped_ifc) ] static {mapped_inline_host_ip | mapped_obj | interface [ipv6 ]} [net-to-net ] [dns | service {tcp | udp | sctp } real_port mapped_port] [no-proxy-arp ]
        while (args.length > 0){
            if (r = (/(static|dynamic)/.exec(args[0]) !== null)) {
                nat.type = r[1]
                if (network.isIPv4(args[1])) nat.mapped = {translated:args[1], syntax:'IP'}
                else if (/interface/.test(args[1])) {
                    nat.mapped = {translated:args[1], syntax:'interface', IPv:4}
                    if (/ipv6/.test(args[2])) {
                        nat.mapped.IPv = 6
                        args.shift()
                    }
                }
                else nat.mapped = {translated:args[1], syntax:'object'}
                args = args.slice(2)
            }
            else if (/net-to-net|no-proxy-arp/.test(args[0])) {
                nat.options.push(args[0])
                args.shift()
            }
            else if (/dns/.test(args[0])) {
                nat.service = {protocol:'dns'}
                args.shift()
            }
            else if (/service/.test(args[0])) {
                nat.service = {protocol:args[1], real:args[2], translated:args[3]}
                args = args.slice(4)
            }
            else args.shift()
        }
        return {object:object, nat:nat}
    }
}
