...
function parseNAT(line,hierarchy,object){               // Receives the configuration line for a NAT and the object affected if applicable
    ...
    object = object || false
    var nat = {}
    nat['srcInterface'] = line.split('(')[1].split(',')[0]
    nat['dstInterface'] = line.split(',')[1].split(')')[0]

    var args = line.split(')')[1].trim().split(' ')
    if (!object) {
        while (args.length > 0){
            if (/source|destination/.test(args[0])){
                nat[args[0]] = {type:args[1], real:args[2], translated:args[3]}
                args = args.slice(4)
            }
